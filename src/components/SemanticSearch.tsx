...
import type { BookWithSearchMetadata, Book } from '@/types';
...
export default function SemanticSearch({ 
  ...
  // isEnterPressed state removed as unused
  const [currentPlaceholder, setCurrentPlaceholder] = useState('');
  const [placeholderIndex, setPlaceholderIndex] = useState(0);
  const [isTyping, setIsTyping] = useState(true);
  const [isFocused, setIsFocused] = useState(false);
  // isScrolled state removed as unused
    ...
    return (
      <div style={{ position: 'relative', width: '100%' }}>
        ...
        {showDropdown && results.length > 0 && (
          ...
          >
            {results.map((bookItem, index) => {
              const book = bookItem as unknown as Book;
              return (
              <div
                key={book.id}
              ...
              >
                ...
                <div style={{ position: 'relative', minWidth: '48px' }}>
                  ...
                  />
                  {bookItem.searchMetadata?.similarity && (
                    <div style={{
                      position: 'absolute',
                      top: '-8px',
                      right: '-8px',
                      ...
                      {Math.round(bookItem.searchMetadata.similarity * 100)}%
                    </div>
                  )}
                </div>

...
export async function POST(request: NextRequest) {
  try {
    ...
    const merchantOrderId = `QR_ORDER_${Date.now()}_${user.id.slice(0, 8)}`
    ...
    if (orderError) {
      console.error('Error creating QR order record:', orderError)
    ...
    try {
      const kbzPayResponse = await kbzPayService.createQROrder(kbzPayOrder)

      if (kbzPayResponse.result !== 'SUCCESS') {
        console.error('KBZPay QR order creation failed:', kbzPayResponse)
        ...
        return NextResponse.json({
          error: 'Failed to create QR payment order',
      ...
      console.error('KBZPay QR API Error:', kbzPayError)
      ...
      return NextResponse.json({
        error: 'QR payment service unavailable. Please try again later.'
      }, { status: 503 })
    }

      ...
      switch (status) {
        case CALLBACK_STATUS.EDITING: // Document editing (user connected/disconnected)
          try {
            ...
            console.log('Activity logging skipped for editing event');
          }
          break;
        ...
        case CALLBACK_STATUS.FORCE_SAVE: // Document force-saved
          if (url) {
            ...
            // Update manuscript record (only update existing columns)
            await supabase
              .from('manuscripts')
              .update({
                file_url: publicUrl,
                updated_at: new Date().toISOString()
                // Note: last_edited_by and editing_status columns may not exist in current schema
              })
              .eq('id', manuscriptId);
        ...
        case CALLBACK_STATUS.FORCE_SAVE_ERROR: // Force saving error
          ...
          try {
            ...
            console.log('Activity logging skipped for save error');
          }
          break;
        ...
        case CALLBACK_STATUS.CLOSED: // Document closed without changes
          try {
            ...
            console.log('Activity logging skipped for editor closed');
          }
          break;

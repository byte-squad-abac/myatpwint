...
export interface KBZPayOrderInfo {
  appid: string
  merch_code: string
  prepay_id: string
  timestamp: number
...
class KBZPayService {
  ...
  async queryOrder(merchantOrderId: string): Promise<KBZPayPrecreateResponse> {
    ...
    try {
      ...
      throw new Error(`Failed to query KBZPay order: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
}

export const kbzPayService = new KBZPayService()
    ...
    try {
      ...
      const response = await fetch(this.WEBHOOK_URL, {
        ...
        headers: {
          ...
          'User-Agent': 'MyatPwint-BookPublishing/1.0'
        },
      ...
      this.logMarketingCampaign(bookData.id, 'success', null, ['facebook', 'twitter', 'email', 'telegram'], result).catch(console.error);
  ...
  static async testConnection(): Promise<N8NResponse> {
    try {
      const testPayload = {
        test: true,
        timestamp: new Date().toISOString(),
        message: 'N8N connection test from MyatPwint app'
      };
